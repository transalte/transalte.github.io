...
There are a number of ways to achieve this, but in this example a new object (technically a subpath) will be used instead for quickness. Add to the patch the **M4L.Bal2~** object and note that when mousing over the inlets and outlets there is no information supplied. The only way to see what this object requires and does it to hold down **cmd** and double-click it with the mouse to open it up like an encapsulated object from before.
...
/d08.png "**omx.peaklim~** between delay lines and mix"
...
/d09.png "Feedback taken from after the limiting stage"
...
The patch is becoming a little more fuller at this point with numerous points of control via UI elements. Keeping in mind that this is still a fairly basic patch, this is a good point to look at wireless, or cordless, connections in Max.

These are the same as the direct connections being used up until now with one slight difference, they are cordless and there can be a lot of one-to-many or many-to-one routing options that come in very handy later on in busier patches. One of the downsides of these connections is that they can be unwieldy to use unless you adopt a proper naming convention to ensure that you know what every connection is. Another negative aspect is that when sending audio signals cordlessly there is a minute latency introduced.

### Implementation
There are two sets of wireless objects; one for data and one for audio.
...
Note that the data send and receive object names can be truncated to simply **s** and **r**.

This is how the connections work:

/d10.png "'Cordless' connections"

In practice, the send and receive objects will work as expected here but there may be an issue when there is more than one instance of a device in an Ableton Live session. Suppose that the **live.dial** and **send** are transmitting data to a filter via the **receive frequency** object and there is an instance of this device on every channel. If the user was to turn the dial on one instance it would change the frequency on every instance within the Ableton Live session. To get around this, and keep any cordless transmission within the confines of each instance, three hyphen can be added to the name of the sends i.e **send ---frequency** and **receive ---frequency**, or in the truncated version **s ---frequency** and **r ---frequency**. This is also the same for the signal rate **send~** and **receive~** objects however note that these cannot be truncated to **s~** and **r~**.
...
/d11.png "Delay Patch with cordless connections"

+++

## Feedback Loop Processing
The final stage of process for this basic delay is to add in the filter from the previous section (or an edited version of it to reflect the mono-signal input of this device). As the previous filter was encapsulated, it should be easy to copy and paste the sub-patch using the standard shortcuts.
...
/d12.png "Filter outside of feedback loop but in signal path"

or in the same position but with the return from from feedback loop also being fed into it:

/d13.png "Filter inside feedback loop and in input signal path to delay line"
